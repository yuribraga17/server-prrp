...
Player_GiveOre(playerid, type, cooldown = 0)
{
    ...
    SendClientMessage(playerid, 0x2ECC71FF, "MINERAÇÃO: {FFFFFF}Você pode pressionar N para soltar seu minério.");
	return 1;
}
...
Vein_Update(id)
{
	...
	format(label, sizeof(label), "%s\n%d/%d%s\n\n/informações de minério - /ore mine", OreData[type][Name], VeinData[id][Amount], OreData[type][Amount], (is_red) ? ("{E74C3C}") : ("{FFFFFF}"));
...
public OnFilterScriptInit()
{
	...
	for(new i; i < sizeof(DropPoints); i++)
	{
	    format(label, sizeof(label), "Ponto de entrega de minério - %s\n\n{FFFFFF}Traga seus minérios aqui para conseguir dinheiro!", DropPoints[i][Location]);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	if(newstate == PLAYER_STATE_DRIVER)
	{
	    ...
	    {
	        new string[128];
	        format(string, sizeof(string), "MINERAÇÃO: {FFFFFF}Este veículo tem {F39C12}%d {FFFFFF}minérios carregados que valem {2ECC71}R$%d.", Vehicle_LoadedOres(vehicleid), Vehicle_GetOreValue(vehicleid));
			SendClientMessage(playerid, 0x2ECC71FF, string);
			SendClientMessage(playerid, 0x2ECC71FF, "MINERAÇÃO: {FFFFFF}Você pode vender seus minérios carregados para soltar pontos marcados por um ícone de caminhão.");
			SendClientMessage(playerid, 0x2ECC71FF, "MINERAÇÃO: {FFFFFF}Você pode usar /vehicle para ver mais informações.");
			for(new i; i < sizeof(DropPoints); i++)
			{
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if((newkeys & KEY_NO) && CarryingOre[playerid] != -1)
	{
		...
		if(id != -1)
		{
		    if(Player_GetClosestOre(playerid, 1.5) != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não pode deixar seu minério aqui.");
			...
			format(label, sizeof(label), "%s Minério\n{FFFFFF}\n\n/minério pegar", OreData[type][Name]);
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
	if(checkpointid == LoadingPoint[playerid])
	{
		...
		if(Vehicle_LoadedOres(vehicleid) >= VEHICLE_LIMIT) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não pode carregar mais minérios neste veículo.");
		LoadedOres[vehicleid][ CarryingOre[playerid] ]++;
		
		new string[48];
		format(string, sizeof(string), "MINERAÇÃO: {FFFFFF}Carregado %s.", OreData[ CarryingOre[playerid] ][Name]);
		SendClientMessage(playerid, 0x2ECC71FF, string);
		ApplyAnimation(playerid, "CARRY", "putdwn05", 4.1, 0, 1, 1, 0, 0, 1);
		Player_CleanUp(playerid, 1);
		return 1;
	...
	if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	{
		for(new i; i < sizeof(DropPoints); i++)
		{
		    if(checkpointid == DropPoints[i][PointCP])
		    {
		        ...
		        format(string, sizeof(string), "MINERAÇÃO: {FFFFFF}Vendido {F39C12}%d {FFFFFF}minérios e ganhou {2ECC71}R$%d.", Vehicle_LoadedOres(vehicleid), cash);
		        SendClientMessage(playerid, 0x2ECC71FF, string);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
	{
		case DIALOG_ORE_TAKE:
		{
		    ...
		    if(MiningVein[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você já está minerando.");
			if(CarryingOre[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você já está carregando um minério.");
			new id = GetNearestVehicleEx(playerid);
			if(!IsValidVehicle(id)) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não está perto de nenhum veículo.");
			new Float: x, Float: y, Float: z;
			GetVehicleBoot(id, x, y, z);
			if(GetPlayerDistanceFromPoint(playerid, x, y, z) > 3.0) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não está perto da traseira do veículo.");
			if(LoadedOres[id][listitem] < 1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Este veículo não tem esse minério carregado.");
...
{
    if(isnull(params)) return SendClientMessage(playerid, 0xE88732FF, "USE: {FFFFFF}/vehicle [minerio/levar]");
    if(!strcmp(params, "minerio", true)) {
        new vehicleid = GetNearestVehicleEx(playerid);
        if(!IsValidVehicle(vehicleid)) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não está perto de nenhum veículo.");
        if(!Vehicle_IsMiningVehicle(vehicleid)) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}O veículo não é um veículo de mineração.");
        new string[196], title[32];
		format(string, sizeof(string), "Nome\tQuantidade\tValor\n");
		...
		format(title, sizeof(title), "Minérios Carregados {E74C3C}(%d/%d)", Vehicle_LoadedOres(vehicleid), VEHICLE_LIMIT);
		ShowPlayerDialog(playerid, DIALOG_ORE_INFO, DIALOG_STYLE_TABLIST_HEADERS, title, string, "Fechar", "");
    }else if(!strcmp(params, "levar")) {
        if(IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não pode fazer isso em um veículo.");
		if(MiningVein[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}You're already mining.");
		if(CarryingOre[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}You're already carrying an ore.");
		new id = GetNearestVehicleEx(playerid);
		if(!IsValidVehicle(id)) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não está perto de nenhum veículo.");
        if(!Vehicle_IsMiningVehicle(id)) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}O veículo não é um veículo de mineração.");
		new Float: x, Float: y, Float: z;
		GetVehicleBoot(id, x, y, z);
		if(GetPlayerDistanceFromPoint(playerid, x, y, z) > 3.0) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não está perto da traseira do veículo.");
        new string[196], title[32];
		format(string, sizeof(string), "Nome\tQuantidade\n");
        for(new i; i < MAX_ORE_TYPES; i++) format(string, sizeof(string), "%s%s\t%d\n", string, OreData[i][Name], LoadedOres[id][i]);
		format(title, sizeof(title), "Carregado minérios {E74C3C}(%d/%d)", Vehicle_LoadedOres(id), VEHICLE_LIMIT);
		ShowPlayerDialog(playerid, DIALOG_ORE_TAKE, DIALOG_STYLE_TABLIST_HEADERS, title, string, "Levar", "Fechar");
    }
...
{
    if(isnull(params)) return SendClientMessage(playerid, 0xE88732FF, "USE: {FFFFFF}/ore [info/mine/take]");
    if(!strcmp(params, "info", true)) {
		new id = Player_GetClosestVein(playerid);
		if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Você não está perto de uma veian.");
        new string[128], type = VeinData[id][Type];
        format(string, sizeof(string), "Nome\t{%06x}%s\nValor\t{2ECC71}$%d\nValor da veia\t{2ECC71}$%d\nTempo de mineração\t%.2f segundos", OreData[type][Color] >>> 8, OreData[type][Name], OreData[type][Value], (OreData[type][Value] * VeinData[id][Amount]), Vein_CalculateTime(id) / 1000);
		ShowPlayerDialog(playerid, DIALOG_ORE_INFO, DIALOG_STYLE_TABLIST, "Informações da mineração", string, "Fechar", "");
    }else if(!strcmp(params, "minerio")) {
        if(!Vehicle_IsMiningVehicle(GetPVarInt(playerid, "LastVehicleID"))) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Your last vehicle isn't available for mining.");
		if(MiningVein[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}You're already mining.");
		if(CarryingOre[playerid] != -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}You're already carrying an ore.");
		new id = Player_GetClosestVein(playerid);
		if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}You're not near any veins.");
		if(VeinData[id][BeingMined]) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}The vein you want to mine is being mined by someone else.");
		if(VeinData[id][Amount] < 1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}The vein you want to mine is empty.");
		new Float: x, Float: y, Float: z;
		GetVehicleBoot(GetPVarInt(playerid, "LastVehicleID"), x, y, z);
		if(GetPlayerDistanceFromPoint(playerid, x, y, z) > 60.0) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Your last vehicle is too far away.");
		...
		if(id == -1) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}You're not near any ores.");
		new Float: x, Float: y, Float: z;
		GetVehicleBoot(GetPVarInt(playerid, "LastVehicleID"), x, y, z);
		if(GetPlayerDistanceFromPoint(playerid, x, y, z) > 60.0) return SendClientMessage(playerid, 0xE74C3CFF, "ERRO: {FFFFFF}Your last vehicle is too far away.");
